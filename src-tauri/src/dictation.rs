...
pub async fn run_clipboard_inner(app: &AppHandle, shortcut_id: &str) -> Result<String, String> {
    ...
    // Run the (optional LLM) pipeline + paste as a DETACHED, abortable task —
    // same shape as stop_inner — so Esc cancels it mid-flight, including
    // aborting an in-flight LLM HTTP call (dropping the future cancels the
    // request). Register Esc first so the HUD is dismissable during processing;
...
async fn process_and_deliver(
    ...
    // Bail before any save / LLM work if this delivery was already superseded
...
async fn run_fluidaudio(
    ...
    // on a notarized release build, which has no stderr — same reason DICT_DIAG
